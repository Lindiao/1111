...
public class ControllerBattle : Controller
{
    ...
    public override void Control()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Z) && !btn && !NoAfk)
        {
            Move(0);
            lockMove = true;
        ...
        if (Input.GetKeyDown(KeyCode.X) && !btn && !NoAfk)
        {
            Shockwave();
        }

        if (Input.GetKeyDown(KeyCode.C) && !btn && !NoAfk)
        {
            Move(0);
            lockMove = true;
    ...
    public override void Shot()
    {
        ...
        if (Game.gift > 0)
        {
            Instantiate(gift, pt.position, pt.rotation);
        }
        else {
